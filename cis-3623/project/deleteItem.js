let find_item_button = document.getElementById("find_item_btn");
    ...
    let delete_item_button = document.getElementById("delete_item_btn");
    delete_item_button.addEventListener("click", async function(event) {
        console.log(event.target.id + " clicked ...");
        let type = document.getElementsByName("type");
        let delete_item_type = "";
        for (let i = 0; i < type.length; i++) {
            if (type[i].checked) {
                delete_item_type = type[i].value;
                break;
            };
        };
        let id_input_text = document.getElementById("input-id");
        console.log("Deleteing item with:", {
            id: id_input_text.value,
        });
